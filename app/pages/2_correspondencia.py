...
def cargar_datos():
    ...
    if is_asignacion:
        return service.listar_correspondencia(ver_todas=True)
    else:
        return service.listar_correspondencia(id_usuario=id_usuario_actual, ver_todas=False)
...
with tab_gestion:
    ...
    datos_corr = cargar_datos()
    
    if not datos_corr:
        st.info("No hay correspondencia para mostrar.")
    else:
        ...
        for c in datos_corr:
            ...
            tabla_datos.append({
                ...
                "Fecha Radicación": formatear_fecha(c.get("fecha_radicacion")),
                "Vencimiento": formatear_fecha(c.get("fecha_vencimiento")),
        ...
        st.divider()
        st.subheader("Gestionar Radicado")
        
        radicados_op = {str(c["_id"]): c.get("numero_radicado") for c in datos_corr}
        id_seleccionado = st.selectbox("Seleccione un radicado para gestionar", options=list(radicados_op.keys()), format_func=lambda x: radicados_op[x])
        
        if id_seleccionado:
            corr_actual = next((c for c in datos_corr if str(c["_id"]) == id_seleccionado), None)
            if corr_actual:
                with st.expander(f"Detalles: {corr_actual.get('numero_radicado')}", expanded=True):
                    ...
                    with col_info2:
                        st.write(f"**Estado Actual:** {corr_actual.get('estado_actual')}")
                        st.write(f"**Responsable Actual:** {corr_actual.get('responsable_actual', {}).get('nombre', 'N/A')}")
                ...
                if can_assign:
                    with col_acc2 if is_asignacion else col_acc1:
                        with st.popover("👥 Asignar / Reasignar"):
                ...
                if es_responsable or is_asignacion:
                    ...
                    with col_acc3 if is_asignacion else (col_acc2 if can_assign else col_acc1):
                        with st.popover("✅ Responder / Cerrar"):
